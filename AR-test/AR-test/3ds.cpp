...
UINT C3DSModel::ReadObjectInfo(t3DObject *pObj, UINT n)
{
	...
	while (count < n)
	{
		...
		case OBJECT_VERTEX:
			...
			pObj->pVerts = new _Vector3[pObj->numOfVerts];
			memset(pObj->pVerts, 0, sizeof(_Vector3) * pObj->numOfVerts);
			// 按块读取顶点坐标值
...
// 计算两向量的叉积
_Vector3 C3DSModel::Cross(_Vector3 v1, _Vector3 v2)
{
	_Vector3 vCross;
...
// 向量单位化
_Vector3 C3DSModel::Normalize(_Vector3 vNormal)
{
	double Magnitude;
...
{
	_Vector3 v1,v2, vNormal,vPoly[3];
	...
	{
		obj = &m_3DModel.pObject[nOfObj];
		_Vector3 *pNormals		= new _Vector3 [obj->numOfFaces];
		_Vector3 *pTempNormals	= new _Vector3 [obj->numOfFaces];
		obj->pNormals			= new _Vector3 [obj->numOfVerts];
		...
		_Vector3 vSum(0.0, 0.0, 0.0);
		_Vector3 vZero(0.0, 0.0, 0.0);
		int shared=0;
